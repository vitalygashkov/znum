...
(async () => {
  ...
  const imagesDir = join(process.cwd(), 'downloads', documentId);
  console.log('Скачивание изображений...');
  const pages = await downloadImages(imagesDir, documentId, info);
  await setTimeout(500);
  console.log('Конвертирование изображений в PDF...');
  const output = join(process.cwd(), 'downloads', `${documentId}.pdf`);
  await convertImagesToPdf(pages, output);
